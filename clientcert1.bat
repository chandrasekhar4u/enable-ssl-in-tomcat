keytool -genkeypair -alias serverkey -keyalg RSA -dname "CN=localhost,OU=Application Development,O=GoSmarter,L=Bangalore,S=KA,C=IN" -keypass password -keystore server.jks -storepass password

keytool -genkeypair -alias client1key -keyalg RSA -dname "CN=client1,OU=Application Development,O=GoSmarter,L=Bangalore,S=KA,C=IN" -keypass password -storepass password -keystore client1.jks
